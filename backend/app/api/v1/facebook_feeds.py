"""
Facebook Explorer — API de monitores por palabra clave/tema o página/perfil.
...
router = APIRouter(prefix="/facebook-feeds", tags=["Facebook Explorer"])

PLATFORM_CODE   = "facebook"
ENTITY_TYPE     = "Monitor Facebook"
VALID_TYPES     = ("keyword", "page")


# ── Schemas ───────────────────────────────────────────────────

class FeedCreate(BaseModel):
    feed_type: str   # 'keyword' | 'page'
    term: str
...
def _feed_to_dict(feed: FacebookFeed, db: Session) -> dict:
...
def list_feeds(
    ...
    feeds = db.query(FacebookFeed).filter(FacebookFeed.active == True).order_by(FacebookFeed.created_at).all()
...
def create_feed(
    ...
    existing = db.query(FacebookFeed).filter(
        FacebookFeed.feed_type == data.feed_type,
        FacebookFeed.term == term,
    ).first()
    if existing:
    ...
    db.add(feed)
    db.flush()
    log_action(db, user.id, "facebook_feed_added", request, "facebook_feeds", feed.id,
...
def toggle_rizoma(
    ...
    feed = db.query(FacebookFeed).filter(FacebookFeed.id == feed_id).first()
    if not feed:
        raise HTTPException(status_code=404, detail="Feed no encontrado")
    feed.is_rizoma = not feed.is_rizoma
    log_action(db, current_user.id, "facebook_feed_rizoma_toggled", request, "facebook_feeds", feed.id,
...
def delete_feed(
    ...
    if not feed:
        raise HTTPException(status_code=404, detail="Feed no encontrado")
    feed.active = False
    log_action(db, current_user.id, "facebook_feed_deleted", request, "facebook_feeds", feed.id,
