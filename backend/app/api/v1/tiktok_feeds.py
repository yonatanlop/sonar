"""
TikTok Explorer — API de monitores por palabra clave, hashtag o creador/usuario.
...
router = APIRouter(prefix="/tiktok-feeds", tags=["TikTok Explorer"])

PLATFORM_CODE = "tiktok"
ENTITY_TYPE   = "Monitor TikTok"
VALID_TYPES   = ("keyword", "hashtag", "creator")


# ── Schemas ───────────────────────────────────────────────────

class FeedCreate(BaseModel):
    feed_type: str   # 'keyword' | 'hashtag' | 'creator'
    term: str
...
def _feed_to_dict(feed: TiktokFeed, db: Session) -> dict:
...
def list_feeds(
    ...
    feeds = db.query(TiktokFeed).filter(TiktokFeed.active == True).order_by(TiktokFeed.created_at).all()
...
def create_feed(
    ...
    existing = db.query(TiktokFeed).filter(
        TiktokFeed.feed_type == data.feed_type,
        TiktokFeed.term == term,
    ).first()
    if existing:
    ...
    db.add(feed)
    db.flush()
    log_action(db, user.id, "tiktok_feed_added", request, "tiktok_feeds", feed.id,
...
def toggle_rizoma(
    ...
    feed = db.query(TiktokFeed).filter(TiktokFeed.id == feed_id).first()
    if not feed:
        raise HTTPException(status_code=404, detail="Feed no encontrado")
    feed.is_rizoma = not feed.is_rizoma
    log_action(db, current_user.id, "tiktok_feed_rizoma_toggled", request, "tiktok_feeds", feed.id,
...
def delete_feed(
    ...
    if not feed:
        raise HTTPException(status_code=404, detail="Feed no encontrado")
    feed.active = False
    log_action(db, current_user.id, "tiktok_feed_deleted", request, "tiktok_feeds", feed.id,
