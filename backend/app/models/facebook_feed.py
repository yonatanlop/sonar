...
class FacebookFeed(Base):
    """
    Monitor de Facebook Explorer.
    Puede ser una palabra clave/tema o una página/perfil por nombre.
    Cada feed tiene una Entity interna auto-creada para que las menciones
    pasen por el pipeline NLP existente.
    """
    __tablename__ = "facebook_feeds"

    id:           Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    feed_type:    Mapped[str]       = mapped_column(String(20),  nullable=False)   # 'keyword' | 'page'
