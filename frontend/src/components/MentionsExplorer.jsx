...
// Valores iniciales de los filtros (compartidos por el menú Menciones y la pestaña del detalle de entidad)
const BASE_FILTERS = {
...
// Texto de una keyword: la expresión si existe; si no, keyword + operador + secundaria
const kwLabel = (k) =>
  (k.keyword_expression || '').trim() ||
  (k.keyword + ((k.keyword_secondary || '').trim() ? ` ${k.logic_op} ${k.keyword_secondary}` : ''))
...
/**
 * Explorador de menciones con filtros y búsqueda semántica.
 *  - Sin props: es el menú "Menciones" (todas las entidades).
 *  - Con `entityId`: pestaña "Menciones" del detalle de una entidad. La entidad queda fija (no hay
 *    selector), aparece el filtro por keyword y cada mención muestra con qué keyword coincidió.
 *  - `preset`: filtros iniciales (p. ej. { sentiment: 'negative', date_from: '2026-09-18' } al venir
 *    de un dato del resumen). `preset.topic_label` es solo el nombre del tema para mostrarlo.
 */
export default function MentionsExplorer({ entityId = null, keywords = [], preset = {}, showHeader = true }) {
  const fixed = !!entityId
  const { topic_label: topicLabel, ...presetFilters } = preset
  const canFeedback = useAuthStore(s => s.isAnalyst() || s.isAdmin())
  const qc = useQueryClient()
  const [filters, setFilters] = useState(() => ({ ...BASE_FILTERS, entity_id: entityId ?? '', ...presetFilters }))

  const hasActiveFilters = Object.entries(filters).some(
    ([k, v]) => k !== 'page' && !(fixed && k === 'entity_id') && v !== '' && v !== false
  )
  const resetFilters = () => setFilters({ ...BASE_FILTERS, entity_id: entityId ?? '' })
  ...
  const { data: entities = [] }   = useQuery({ queryKey: ['entities'], queryFn: fetchEntities, enabled: !fixed })
      ...
      {searchMode === 'filters' ? (
        <>
          ...
          <div className="card py-5 space-y-4">
            ...
            <FilterSection label={fixed ? 'Plataforma & Keyword' : 'Entidad & Plataforma'}>
              {!fixed && (
              ...
              {fixed && keywords.length > 0 && (
                <select className="input w-auto max-w-xs" value={filters.keyword_id}
                  onChange={e => setFilter('keyword_id', e.target.value)}
              ...
              {filters.topic_id !== '' && (
                <button type="button" onClick={() => setFilter('topic_id', '')}
                  className="badge bg-blue-100 text-blue-800 flex items-center gap-1"
                  title="Quitar el filtro de tema">
                  Tema: {topicLabel ?? filters.topic_id} <X className="w-3 h-3" />
                </button>
              )}
            </FilterSection>
            ...
            <FilterSection label="Contenido">
              <select className="input w-auto" value={filters.sentiment}
                ...
                <option value="very_negative">Muy negativo</option>
                <option value="negative">Negativo</option>
                <option value="negative_only">Solo negativo (sin muy negativo)</option>
                <option value="neutral">Neutral</option>
                <option value="positive">Positivo</option>
              </select>
