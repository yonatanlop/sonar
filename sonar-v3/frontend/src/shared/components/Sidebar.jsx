...
import {
  LayoutDashboard, Building2, MessageSquare, Bell, FileText,
  Users, Settings2, Radio, UserCircle, Activity, Scale, Network,
} from 'lucide-react'
...
const NAV_SECTIONS = (isAdmin, isAnalyst, unread) => [
  ...
  },
  {
    ...
    items: [
      ...
      ...(isAnalyst ? [{ to: '/settings/rules', icon: Settings2, label: 'Reglas' }] : []),
    ],
  },
  {
    label: 'ANÁLISIS',
    items: [
      { to: '/intelligence', icon: Activity, label: 'Inteligencia' },
      { to: '/actor-map',    icon: Network,  label: 'Mapa de Actores' },
      { to: '/legal',        icon: Scale,    label: 'Casos Legales' },
    ],
  },
  {
    label: 'HERRAMIENTAS',
  ...
  ...(isAdmin ? [{
    ...
    items: [{ to: '/admin/users', icon: Users, label: 'Usuarios' }],
  }] : []),
]
      ...
      <aside className={clsx(
        ...
        <div className="flex items-center gap-3 px-5 py-5 border-b border-primary-700">
          ...
          <div>
            ...
            <p className="text-primary-300 text-xs leading-none">v3 · Hexagonal</p>
          </div>
        </div>
