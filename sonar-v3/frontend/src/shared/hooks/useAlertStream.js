...
export function useAlertStream() {
  ...
  useEffect(() => {
    ...
    const source = new EventSource(`/api/v3/alerts/stream?token=${token}`)
    ...
    source.addEventListener('new_alert', (e) => {
      try {
        ...
        if (alert.severity === 'critical') toast.error(msg, { duration: 8000 })
        else if (alert.severity === 'high') toast.error(msg, { duration: 6000, style: { background: '#ea580c', color: '#fff' } })
